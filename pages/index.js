...
// ── NAVBAR ───────────────────────────────────────────────────────
function GoogleSignInButton({ style }) {
  return (
    <button
      ...
      style={{
        ...
        background:'#fff', color:'#1a1a2e', padding:'0 14px', height:34, fontWeight:600,
        ...(style||{})
      }}
  ...
  return (
    <nav style={{
      ...
      <div style={{marginLeft:'auto',display:'flex',gap:8,alignItems:'center'}}>
        ...
        {session ? (
          <>
            <button style={btnOrange} onClick={onRegister}>🏅 &nbsp;Записаться на марафон</button>
            {isAdmin
            ...
            <button style={{...btnNav,color:'#8C8CA5'}} onClick={() => signOut({callbackUrl:'/'})} title="Выйти">⏻</button>
          </>
        ) : (
          <GoogleSignInButton />
        )}
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.7)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200}}
      ...
      <div style={{background:'#14142A',border:'1px solid #2D2D46',borderRadius:8,width:'min(860px,95vw)',maxHeight:'90vh',overflow:'hidden',display:'flex',flexDirection:'column'}}>
        ...
        <div style={{flex:1,overflowY:'auto',padding:'16px 30px 20px'}}>
          <div style={{fontSize:20,fontWeight:700,color:'#F4A33C',textAlign:'center',marginBottom:4}}>{isEdit ? 'Редактирование' : 'Запись на марафон'}</div>
          <div style={{fontSize:10,color:'#8C8CA5',textAlign:'center',marginBottom:14}}>Заполните все поля для участия в марафоне 2026</div>
    ...
    <div style={{display:'flex',flexDirection:'column',minHeight:'100vh'}}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'16px 24px'}}>
        ...
        <div style={{display:'flex',gap:10,justifyContent:'center',marginBottom:16}}>
          {session ? (
            <button style={{...btnOrange,height:40,fontSize:12}} onClick={()=>{setEditTarget(null);setRegisterOpen(true)}}>🏅 &nbsp;Записаться на марафон</button>
          ) : (
            <GoogleSignInButton style={{height:40,fontSize:12}} />
          )}
